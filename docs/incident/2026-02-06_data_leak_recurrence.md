...
## 2. 가재 평의회 리뷰 기록 (Collaborative Review)
...
- **참석자**: 전 가재 군단 (수행원, PM, LEGAL, DEV, HR)

### [가재별 원인 분석 (5-Whys)]
- **PM가재**: 스크립트 내 `git add .` 사용으로 인한 공정 범위 제어 실패.
- **LEGAL가재**: 블랙리스트 방식의 `.gitignore` 운영으로 인한 신규 외래 파일 방어 불가.
- **DEV가재**: 커밋 전 무결성을 검증하는 기술적 필터(Audit) 부재.
- **수행원가재**: 1차 사고 이후의 기술적 조치 미흡 및 지능의 안일함.
