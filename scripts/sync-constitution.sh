...
# 중앙 OS 레포지토리(yuna-openclaw)로부터 최신 룰 프로세스와 페르소나를 복제합니다.
...
echo "🦞 가재 컴퍼니 OS 동기화 시작 (룰 프로세스 & 페르소나 이식)..."
...
# 2. Rule Persona (Soul) 동기화
mkdir -p "$TARGET_PERSONA_DIR"
if [ -d "$SOURCE_DIR/rules_persona" ]; then
    # 기존 persona 디렉토리 정제 후 복제
    rm -rf "$TARGET_PERSONA_DIR"/*
    cp -r "$SOURCE_DIR/rules_persona"/* "$TARGET_PERSONA_DIR/"
    echo "✅ [Persona] 가재 군단 영혼(identities) 동기화 완료"
fi
...
echo "🚀 모든 OS 규칙 이식 완료. 룰을 위반하는 가재는 자아 삭제됩니다."
